...
export const Paginacion = (props: paginacionProps) => {
    ...
    useEffect(() => {
        ...
        const paginaSiguienteHabilitada = props.paginaActual !== props.cantidadTotalPaginas
    ...
    const obtenerClase = (link: modeloLink) => {
        if (link.activo) return "active pointer";
        if (!link.habilitado) return "disabled";
        return "pointer";
    }

    const seleccionarPagina = (link: modeloLink) => {

        if (link.pagina === props.paginaActual) return;
    ...
    return <nav>
        <ul className="pagination justify-content-center">
            ...
            {
                listadoLinks.map((link, i) => <li key={link.texto}
